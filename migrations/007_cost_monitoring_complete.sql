-- Migration 007: Cost Monitoring System (Complete)
-- API 使用追踪、成本监控、预算告警 - 完整版本（包含清理）

-- 清理旧的对象（如果存在）
DROP FUNCTION IF EXISTS aggregate_daily_cost_metrics(DATE);
DROP FUNCTION IF EXISTS get_user_api_usage_stats(UUID, DATE, DATE);
...
-- 创建索引
CREATE INDEX idx_cost_alerts_type ON cost_alerts (type);
...
-- 创建函数：获取用户 API 使用统计
CREATE FUNCTION get_user_api_usage_stats(
  p_user_id UUID,
  p_start_date DATE,
  p_end_date DATE
)
...
-- 创建函数：获取每日成本汇总
CREATE FUNCTION aggregate_daily_cost_metrics(p_date DATE)
RETURNS void AS $$
DECLARE
