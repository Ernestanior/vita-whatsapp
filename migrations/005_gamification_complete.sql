-- Migration 005: Gamification System (Complete)
-- 社交和激励机制 - 完整版本（包含清理）

-- 清理旧的对象（如果存在）
DROP FUNCTION IF EXISTS update_weekly_goal_progress(UUID, VARCHAR, INTEGER);
...
-- 创建函数：更新每周目标进度
CREATE FUNCTION update_weekly_goal_progress(
  p_user_id UUID,
  p_goal_type VARCHAR,
